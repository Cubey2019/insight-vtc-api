...
var Address = require('bcoin').address;
...
Common.prototype.translateInputAddresses = function(addresses, network) {
  var self = this;
  ...
  function check(addresses) {
    ...
    for(var i = 0; i < addresses.length; i++) {
      try {
        Address.fromString(addresses[i], network);
      } catch(e) {
        throw new Error('Invalid address: ' + addresses[i]);
      }
    }
  }
